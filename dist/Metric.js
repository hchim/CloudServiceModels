...
var _ReduceFunctions = require('./ReduceFunctions');
...
metricSchema.statics.distinctMetrics = function (query, callback, page, numPerPage) {
    ...
    aggregate.match(query).group({ _id: '$tag' }).limit(numPerPage).skip(numPerPage * page).exec(callback);
};
...
metricSchema.statics.searchMetrics = function (query, callback, page, numPerPage) {
    ...
    this.find(query).limit(numPerPage).skip(numPerPage * page).sort({ createTime: -1 }).exec(callback);
};
...
metricSchema.statics.mapReduceQuery = function (query, callback, map, reduce) {
    this.mapReduce({
        query: query,
        sort: { createTime: 1 },
        map: map ? map : _MapFunctions.metric5MMap,
        reduce: reduce ? reduce : _ReduceFunctions.countMetricReduce
    }, callback);
};

exports.default = metricSchema;